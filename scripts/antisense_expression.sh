...
cd plot/$species_directory;
	...
	then
		...
		if [ ! $plot.Antisense-rfam-filtered.expression ]; then gff_plot2median.py -g /home/suu13/projects/antisense/core_fasta/$embl_name*/$embl_name-rfam-filtered.gff -p $plot -a 1> $plot.Antisense-rfam-filtered.expression 2> /dev/null; fi;
