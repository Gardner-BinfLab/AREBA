...
echo -e "Species\tEMBL\tExperiment_ID\tDataset_ID\tCore_CDS_On\tCore_Rfam_On"
...
cd plot/$species_directory;
	...
	then
		...
		if [ ! -f $plot.Antisense.HMM_Core.expression ]; then gff_plot2median.py -g /home/suu13/projects/antisense/core_fasta/$embl_name*/$embl_name.HMM_Core.gff -p $plot -a 1> $plot.Antisense.HMM_Core.expression 2> /dev/null; fi;
		if [ ! -f $plot.Antisense-rfam-filtered.expression ]; then gff_plot2median.py -g /home/suu13/projects/antisense/core_fasta/$embl_name*/$embl_name-rfam-filtered.gff -p $plot -a 1> $plot.Antisense-rfam-filtered.expression 2> /dev/null; fi;

		Core_CDS=$(cat $plot.Antisense.HMM_Core.expression | awk 'BEGIN{FS="$"; total_c=0; treshold_c=0;}{total_c++; if($7>1) treshold_c++;}END{print treshold_c/total_c}');
		Core_Rfam=$(cat $plot.Antisense-rfam-filtered.expression | awk 'BEGIN{FS="$"; total_c=0; treshold_c=0;}{total_c++; if($7>1) treshold_c++;}END{print treshold_c/total_c}');
		echo -e "$species\t$embl_name\t$experiment_id\t$dataset_id\t$Core_CDS\t$Core_Rfam";
	fi;
	done
